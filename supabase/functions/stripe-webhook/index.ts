    ...
    switch (event.type) {
      ...
      case "checkout.session.completed": {
        ...
        if (plan === "report_payg") {
          ...
          if (scanData?.score) {
            await sendReportEmail(scanData, session.customer_email as string, supabase, resendKey, fromEmail, fromName);
          }
...
// ─── Send Report Email ───
async function sendReportEmail(scanData: Record<string, unknown>, email: string, supabase: ReturnType<typeof createClient>, resendKey: string, fromEmail: string, fromName: string) {
